...
# --- default
LAYOUT := qwerty-red-mod
# --- all
LAYOUTS := qwerty-kinesis-mod dvorak-kinesis-mod colemak-symbol-mod workman-p-kinesis-mod qwerty-red-mod
...
GIT_COMMIT_ID := $(shell git log -n 1 | cut -d' ' -f7)

# name to use for the final distribution file or package
TARGET := ergodox-firmware--$(GIT_BRANCH)--$(shell $(DATE_PROG) -d "$(GIT_COMMIT_DATE)" +'%Y%m%dT%H%M%S')--$(GIT_COMMIT_ID)--$(LAYOUT)
